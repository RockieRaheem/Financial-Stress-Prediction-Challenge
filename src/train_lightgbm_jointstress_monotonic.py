"""Train monotonic LightGBM on the 100 strongest joint-stress features."""
...
INCREASING_RISK_FEATURES = {
    "risk_max_all",
    "risk_max_balance_income",
    "risk_max_balance_out_income_change",
    "risk_max_balance_recent_out_income",
    "risk_mean_all",
    "risk_pct_balance",
    "risk_pct_recent_out_income",
    "risk_product_balance_income",
    "risk_product_balance_out_income_change",
    "risk_top2_mean",
    "risk_top3_mean",
}
DECREASING_RISK_FEATURES = {
    "joint_balance_ratio",
    "joint_income_ratio",
    "joint_log_balance_ratio",
    "joint_log_income_ratio",
}
...
def main() -> None:
    ...
    pd.DataFrame({ID_COLUMN: train[ID_COLUMN], TARGET: y, "prediction": oof}).to_csv(
        ARTIFACT_DIR / "lightgbm_jointstress_monotonic_oof.csv", index=False
    )
    pd.DataFrame({"feature": selected, "importance": importances}).sort_values(
        "importance", ascending=False
    ).to_csv(
        ARTIFACT_DIR / "lightgbm_jointstress_monotonic_importance.csv", index=False
    )
    (ARTIFACT_DIR / "lightgbm_jointstress_monotonic_metrics.json").write_text(
    ...
    submission.to_csv(
        SUBMISSION_DIR / "lightgbm_jointstress_monotonic_100.csv", index=False
    )
    print(json.dumps(metrics, indent=2))
    print("Saved submissions/lightgbm_jointstress_monotonic_100.csv")
