"""Train five-fold LightGBM with log-scale financial stress features."""
...
def main() -> None:
    ...
    pd.DataFrame({ID_COLUMN: train[ID_COLUMN], TARGET: y, "prediction": oof}).to_csv(
        ARTIFACT_DIR / "lightgbm_logstress_oof.csv", index=False
    )
    pd.DataFrame({"feature": X.columns, "importance": importances}).sort_values(
        "importance", ascending=False
    ).to_csv(ARTIFACT_DIR / "lightgbm_logstress_importance.csv", index=False)
    (ARTIFACT_DIR / "lightgbm_logstress_metrics.json").write_text(
    ...
    submission.to_csv(SUBMISSION_DIR / "lightgbm_logstress.csv", index=False)
    print(json.dumps(metrics, indent=2))
    print("Saved submissions/lightgbm_logstress.csv")
