"""Train LightGBM on temporal liquidity features with out-of-fold evaluation."""
...
def main() -> None:
    ...
    pd.DataFrame({ID_COLUMN: train[ID_COLUMN], TARGET: y, "prediction": oof}).to_csv(
        ARTIFACT_DIR / "lightgbm_oof.csv", index=False
    )
    pd.DataFrame({"feature": X.columns, "importance": importances}).sort_values(
        "importance", ascending=False
    ).to_csv(ARTIFACT_DIR / "lightgbm_importance.csv", index=False)
    (ARTIFACT_DIR / "lightgbm_metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
    submission = sample.copy()
    assert submission[ID_COLUMN].tolist() == test[ID_COLUMN].tolist()
    submission["Target"] = np.clip(test_predictions, 1e-6, 1 - 1e-6)
    assert not submission.isna().any().any()
    submission.to_csv(SUBMISSION_DIR / "lightgbm_temporal.csv", index=False)
    print(json.dumps(metrics, indent=2))
    print("Saved submissions/lightgbm_temporal.csv")
