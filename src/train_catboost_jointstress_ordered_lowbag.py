"""Train ten-fold Ordered CatBoost with reduced Bayesian bagging noise."""
...
def main() -> None:
    ...
    pd.DataFrame({ID_COLUMN: train[ID_COLUMN], TARGET: y, "prediction": oof}).to_csv(
        ARTIFACT_DIR / "catboost_jointstress_ordered_lowbag_oof.csv", index=False
    )
    (ARTIFACT_DIR / "catboost_jointstress_ordered_lowbag_metrics.json").write_text(
    ...
    print("Saved submissions/catboost_jointstress_ordered_lowbag_10fold_100.csv")
