"""Train Ordered CatBoost on the 100 strongest joint-stress features."""
...
def main() -> None:
    ...
    pd.DataFrame({ID_COLUMN: train[ID_COLUMN], TARGET: y, "prediction": oof}).to_csv(
        ARTIFACT_DIR / "catboost_jointstress_ordered_oof.csv", index=False
    )
    (ARTIFACT_DIR / "catboost_jointstress_ordered_metrics.json").write_text(
    ...
    submission.to_csv(
        SUBMISSION_DIR / "catboost_jointstress_ordered_100.csv", index=False
    )
    print(json.dumps(metrics, indent=2))
    print("Saved submissions/catboost_jointstress_ordered_100.csv")
