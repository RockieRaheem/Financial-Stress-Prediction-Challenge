"""Train five-fold LightGBM on the 100 strongest joint-stress features."""
...
def main() -> None:
    ...
    categorical = [
        column for column in featured.select_dtypes(exclude="number").columns
    ...
    pd.DataFrame({ID_COLUMN: train[ID_COLUMN], TARGET: y, "prediction": oof}).to_csv(
        ARTIFACT_DIR / "lightgbm_jointstress_pruned_oof.csv", index=False
    )
    pd.DataFrame({"feature": selected, "importance": importances}).sort_values(
        "importance", ascending=False
    ).to_csv(ARTIFACT_DIR / "lightgbm_jointstress_pruned_importance.csv", index=False)
    (ARTIFACT_DIR / "lightgbm_jointstress_pruned_metrics.json").write_text(
    ...
    submission.to_csv(
        SUBMISSION_DIR / "lightgbm_jointstress_pruned_100.csv", index=False
    )
    print(json.dumps(metrics, indent=2))
    print("Saved submissions/lightgbm_jointstress_pruned_100.csv")
