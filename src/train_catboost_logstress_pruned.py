"""Train CatBoost on the 300 strongest log-stress features."""
...
def main() -> None:
    ...
    pd.DataFrame({ID_COLUMN: train[ID_COLUMN], TARGET: y, "prediction": oof}).to_csv(
        ARTIFACT_DIR / "catboost_logstress_pruned_oof.csv", index=False
    )
    (ARTIFACT_DIR / "catboost_logstress_pruned_metrics.json").write_text(
    ...
    submission.to_csv(SUBMISSION_DIR / "catboost_logstress_pruned_300.csv", index=False)
    print(json.dumps(metrics, indent=2))
    print("Saved submissions/catboost_logstress_pruned_300.csv")
