"""Blend independent ten-fold Ordered CatBoost repeats into the best ensemble."""
...
REPEAT_WEIGHTS = [0.0, 0.25, 0.5, 0.75, 1.0]
...
REPEAT_OOF = ARTIFACT_DIR / "catboost_jointstress_ordered_10fold_repeat_oof.csv"
REPEAT_TEST = SUBMISSION_DIR / "catboost_jointstress_ordered_10fold_repeat_100.csv"
...
def main() -> None:
    ...
    repeated_tenfold_oof = repeat_oof["prediction"].to_numpy()
    repeated_tenfold_test = repeat_test["Target"].to_numpy()
    ...
    for repeat_weight in REPEAT_WEIGHTS:
        tenfold_oof_predictions = (
            (1.0 - repeat_weight) * original_tenfold_oof
            + repeat_weight * repeated_tenfold_oof
        )
        tenfold_test_predictions = (
            (1.0 - repeat_weight) * original_tenfold_test
            + repeat_weight * repeated_tenfold_test
        )
        ordered_oof = (
    ...
    anchor_test_difference = float(
        np.max(
            np.abs(
                control_test_predictions - saved_anchor_test["Target"].to_numpy()
            )
        )
    )
    ...
    repeat_percent = int(round(100 * best_weight))
    filename = (
        f"highdata_jointstress_repeatordered_r{repeat_percent:03d}_"
    ...
    ).to_csv(ARTIFACT_DIR / "repeated_ordered_ensemble_oof.csv", index=False)
    metrics = {
        "repeat_weights_tested": REPEAT_WEIGHTS,
        "selected_repeat_weight": best_weight,
        "results": results,
    ...
    (ARTIFACT_DIR / "repeated_ordered_ensemble_metrics.json").write_text(
