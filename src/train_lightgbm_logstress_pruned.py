"""Train five-fold LightGBM on the 300 strongest log-stress features."""
...
def main() -> None:
    ...
    featured = add_temporal_features(combined, include_log_stress=True)
    categorical = [
    ...
    pd.DataFrame({ID_COLUMN: train[ID_COLUMN], TARGET: y, "prediction": oof}).to_csv(
        ARTIFACT_DIR / "lightgbm_logstress_pruned_oof.csv", index=False
    )
    pd.DataFrame({"feature": selected, "importance": importances}).sort_values(
        "importance", ascending=False
    ).to_csv(ARTIFACT_DIR / "lightgbm_logstress_pruned_importance.csv", index=False)
    (ARTIFACT_DIR / "lightgbm_logstress_pruned_metrics.json").write_text(
    ...
    submission.to_csv(SUBMISSION_DIR / "lightgbm_logstress_pruned_300.csv", index=False)
    print(json.dumps(metrics, indent=2))
    print("Saved submissions/lightgbm_logstress_pruned_300.csv")
